...
vi.mock('../../src/database/prisma.js', () => ({
    prisma: {
        market: {
            create: vi.fn((args) => Promise.resolve({
                id: 'test-market-uuid',
...
describe('POST /api/markets - Create Market', () => {
    ...
    it('should create a market successfully with valid data', async () => {
        const marketData = {
            ...
            description: 'This market predicts whether Bitcoin will reach $100,000 USD by December 31, 2026.',
    ...
    it('should use default resolution time if not provided', async () => {
        ...
        const expectedResolutionTime = new Date(closingDate.getTime() + 24 * 60 * 60 * 1000);
        expect(factoryService.createMarket).toHaveBeenCalledWith(
            expect.objectContaining({
