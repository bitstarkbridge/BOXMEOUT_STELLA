...
import { PrismaClient, MarketStatus, TradeType, TradeStatus } from '@prisma/client';
...
describe('Market Lifecycle E2E', () => {
  ...
  it('should complete full market lifecycle', async () => {
    ...
    await prisma.user.update({
      where: { id: testUser.id },
      data: {
        usdcBalance: { decrement: Number(trade.totalAmount) + Number(trade.feeAmount) }
      }
    });
    ...
    expect(Number(balanceAfterTrade!.usdcBalance)).toBeLessThan(Number(balanceBefore!.usdcBalance));
    ...
    expect(Number(finalBalance!.usdcBalance)).toBeGreaterThan(Number(balanceAfterTrade!.usdcBalance));
